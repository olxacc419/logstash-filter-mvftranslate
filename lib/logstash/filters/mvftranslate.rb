...
class LogStash::Filters::MVFTranslate < LogStash::Filters::Base
  config_name "mvftranslate"

  # The name of the logstash event field containing the value to be compared for a
  # match by the translate filter (e.g. `message`, `host`, `response_code`). 
  # 
  # If this field is an array, all the values will be used.
  config :field, :validate => :string, :required => false

  # The name of the logstash event field in an array containing the value to be compared for a
  # match by the translate filter (e.g. `message`, `host`, `response_code`). 
	config :sub_field, :validate => :string, :required => false
  ...
  # The destination field in an array you wish to populate with the translated code.
	config :sub_destination, :validate => :string, :required => false
  ...
  def filter(event)
    ...
    begin
      file = open("/home/ms/logstash-2.1.0/outpput.txt", "a")
      file.write("Started Writing" + "\n")
      if event[@field].is_a?(Array)
				file.write(event[@field])
        #if array
				matched = false
        event[@field].each_with_index do |item, i| #for
	  			source = item[@sub_field].to_s
	  			file.write("ProductId: " + source + "\n")
	 				# file.write("Dictionary: " + @dictionary + "\n")
          if @exact # if exact
            if @regex
              key = @dictionary.keys.detect{|k| source.match(Regexp.new(k))}
              if key
                event[@destination[i][@sub_destination]] = @dictionary[key]
                matched = true
              end
            elsif @dictionary.include?(source)
	      			file.write("Mapping: " + @dictionary[source] + "\n")
	      			# file.write(" to: " + event[@destination] + "\n")
	      			event[@destination][i][@sub_destination] = {}
              event[@destination][i][@sub_destination] = @dictionary[source]
	      			file.write("Mapped: " + event[@destination][i][@sub_destination] + "\n")
              matched = true
            end
          else # else not exact
            translation = source.gsub(Regexp.union(@dictionary.keys), @dictionary)
            if source != translation
              event[@destination[i][@sub_destination]] = translation.force_encoding(Encoding::UTF_8)
              matched = true
            end
          end # end exact

          if not matched and @fallback
            event[@destination[i][@sub_destination]] = event.sprintf(@fallback)
            matched = true
          end
        end # for
